...
async def terminal_ws(websocket: WebSocket, task_id: int, token: str = Query("")):
    ...
    try:
        ...
        enter_container_cmd = (
            ...
            f'  echo "[ones-AI] 自动追踪 AI 处理过程 (Ctrl+C 可切到交互Shell)"; '
            f'  exec docker exec -i "$CONTAINER" /bin/bash -c \''
            f'    echo "=== AI 实时处理输出 ==="; '
            f'    MAIN_PID=$(cat /proc/1/status 2>/dev/null | head -1 | awk "{{print \\$2}}" || echo 1); '
            f'    if [ -f /proc/1/fd/1 ]; then '
            f'      tail -f /proc/1/fd/1 2>/dev/null; '
            f'    elif [ -f /proc/$MAIN_PID/fd/1 ]; then '
            f'      tail -f /proc/$MAIN_PID/fd/1 2>/dev/null; '
            f'    else '
            f'      echo "[ones-AI] 无法追踪主进程输出，已进入交互 Shell"; '
            f'      exec /bin/bash; '
            f'    fi'
            f'  \'; '
            f'else '
